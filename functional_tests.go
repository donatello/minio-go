...
func testFunctional() {
	...
	function = "Presign(method, bucketName, objectName, expires, reqParams, extraHeaders)"
	functionAll += ", " + function
	...
	presignedURL, err := c.Presign(context.Background(), "PUT", bucketName, objectName+"-presign-custom", 3600*time.Second, nil, presignExtraHeaders)
	if err != nil {
...
func testFunctionalV2() {
	...
	_, err = c.Presign(context.Background(), "PUT", bucketName, objectName+"-presign-custom", 3600*time.Second, nil, presignExtraHeaders)
	if err == nil {
